...
struct ViewSmoothingData_t
{
	...
	Vector	vecAnglesSaved;
	Vector	vecOriginSaved;
	Vector	vecAngleDiffSaved;	// The original angular error between the entry/exit anim and player's view when we started playing the anim.
	Vector	vecAngleDiffMin;	// Tracks the minimum angular error achieved so we can converge on the anim's angles.
};
