...
BEGIN_DATADESC( CBaseMonster )
	...
	DEFINE_AUTO_ARRAY( m_hOldEnemy, FIELD_EHANDLE ),
	DEFINE_AUTO_ARRAY( m_vecOldEnemy, FIELD_POSITION_VECTOR ),
...
int CBaseMonster :: CheckEnemy ( CBaseEntity *pEnemy )
{
	...
	if ( HasConditions( bits_COND_SEE_ENEMY ) )
	{
		...
		if (pEnemy->GetAbsVelocity() != g_vecZero)
		{
...
float CBaseMonster :: OpenDoorAndWait( entvars_t *pevDoor )
{
	...
	if (pcbeDoor && !pcbeDoor->IsLockedByMaster())
	{
		...
		flTravelTime = pcbeDoor->GetMoveDoneTime() - pcbeDoor->GetLocalTime();
...
// Debug, Draw the triangulation
		...
		if ( CheckLocalMove( vecOrigin, vecRight, pTargetEnt, NULL ) == LOCALMOVE_VALID )
		{
		...
		if ( CheckLocalMove( vecOrigin, vecLeft, pTargetEnt, NULL ) == LOCALMOVE_VALID )
		{
		...
		if (pev->movetype == MOVETYPE_FLY)
		{
			if ( CheckLocalMove( vecOrigin, vecTop, pTargetEnt, NULL ) == LOCALMOVE_VALID)
			{
...
#if 1
			if ( CheckLocalMove( vecOrigin, vecBottom, pTargetEnt, NULL ) == LOCALMOVE_VALID )
			{
...
void CBaseMonster :: SetEyePosition ( void )
{
	...
	if( GetEyePosition( pmodel, vecEyePosition ))
	{
...
void CBaseMonster :: StepSound( void )
{
	...
	case CHAR_TEX_CONCRETE:
		...
		rgsz[1] = "player/pl_step3.wav";
		rgsz[2] = "player/pl_step2.wav";
	...
	case CHAR_TEX_METAL:
		...
		rgsz[1] = "player/pl_metal3.wav";
		rgsz[2] = "player/pl_metal2.wav";
	...
	case CHAR_TEX_DIRT:
		...
		rgsz[1] = "player/pl_dirt3.wav";
		rgsz[2] = "player/pl_dirt2.wav";
	...
	case CHAR_TEX_VENT:
		...
		rgsz[1] = "player/pl_duct3.wav";
		rgsz[2] = "player/pl_duct2.wav";
		rgsz[3] = "player/pl_duct4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_GRATE:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "player/pl_grate1.wav";
		rgsz[1] = "player/pl_grate3.wav";
		rgsz[2] = "player/pl_grate2.wav";
	...
	{
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "player/pl_wade1.wav";
		rgsz[1] = "player/pl_wade3.wav";
		rgsz[2] = "player/pl_wade2.wav";
		rgsz[3] = "player/pl_wade4.wav";
		cnt = 4;
	}
...
Vector CBaseMonster :: GetGunPosition( )
{
	...
	Vector vecSrc = GetAbsOrigin() + gpGlobals->v_forward * m_HackedGunPos.y
	...
	for ( i = 0; i < WorldGraph.m_cNodes ; i++ )
	{
		...
		if ( node.m_sHintType )
		{
			...
			if ( FValidateHintType ( node.m_sHintType ) )
			{
				...
				{
					UTIL_TraceLine ( EyePosition(), node.m_vecOrigin + pev->view_ofs, ignore_monsters, edict(), &tr );
...
public:
	...
	STATE GetState( void ) { return pev->health ? STATE_ON : STATE_OFF; };
...
void CMonsterTarget :: KeyValue( KeyValueData* pkvd)
{
	if( FStrEq( pkvd->szKeyName, "m_iClass" ))
	{
		pev->frags = Q_atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else
	{
