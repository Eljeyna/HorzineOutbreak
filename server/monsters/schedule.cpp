	...
	case TASK_PLAY_SEQUENCE_FACE_TARGET:
		{
			...
			if ( pTarget )
			{
				pev->ideal_yaw = UTIL_VecToYaw( pTarget->GetAbsOrigin() - GetLocalOrigin());
				ChangeYaw( pev->yaw_speed );
			}
...
//				return;
	...
	case TASK_FIND_COVER_FROM_ORIGIN:
		{
			if ( FindCover( GetLocalOrigin(), pev->view_ofs, 0, CoverRadius() ) )
			{
				// then try for plain ole cover
				m_flMoveWaitFinished = gpGlobals->time + pTask->flData;
				TaskComplete();
	...
	case TASK_GET_PATH_TO_ENEMY_LKP:
		{
			...
			else if (BuildNearestRoute( m_vecEnemyLKP, pev->view_ofs, 0, (m_vecEnemyLKP - GetLocalOrigin()).Length() ))
			{
				TaskComplete();
			}
			else
	...
	case TASK_FACE_SCRIPT:
		{
			if ( m_hTargetEnt != NULL )
			{
				pev->ideal_yaw = UTIL_AngleMod( m_hTargetEnt->GetLocalAngles().y );
			}
