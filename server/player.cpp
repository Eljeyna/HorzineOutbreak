...
BEGIN_DATADESC( CBasePlayer )
	...
	DEFINE_AUTO_ARRAY( m_hKeyCatchers, FIELD_EHANDLE ),
...
void LinkUserMessages( void )
{
	...
	gmsgAmmoX = REG_USER_MSG("AmmoX", 4);
...
void CBasePlayer::StartDeathCam( void )
{
	...
	else
	{
		...
		StartObserver( tr.vecEndPos, UTIL_VecToAngles( tr.vecEndPos - GetAbsOrigin()));
		return;
	}
}
...
void CBasePlayer :: UpdateKeyCatchers( void )
{
	...
	{
		// make sure what key catcher is active
...
void CBasePlayer::DropHoldableItem( void )
{
	...
	pObject->SetAbsVelocity( GetAbsVelocity() );
...
void CBasePlayer :: UpdateClientData( void )
{
	if (m_fInitHUD)
	{
		...
		UTIL_FireTargets( "game_playerspawn", this, this, USE_TOGGLE, 0 );
...
void CStripWeapons :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer )
		pPlayer->RemoveAllItems( FBitSet( pev->spawnflags, SF_REMOVE_SUIT ), FBitSet( pev->spawnflags, SF_REMOVE_CYCLER ));
}
